...
import { AdCategoryForm, AdCategoryInput, AdReviewBodyTr, AdReviewEmpty, AdReviewHeadTr, AdReviewHeader, AdReviewItemList, AdReviewLeftSide, AdReviewListWrap, AdReviewMidSide, AdReviewRightSide, AdReviewTable, AdReviewTbody, AdReviewTd, AdReviewTh, AdReviewThead, AdReviewWrap } from '../ItemManage/reviewStyle';
...
const submitAddHandler = async (pk) => {
  ...
  const handleViewCategories = async (pk) => {
    const productReview = await axios.get(`/products/productAllParentsKinds/${pk}`, {
      headers: { 'Content-Type': 'application/json' },
      withCredentials: true,
    });
